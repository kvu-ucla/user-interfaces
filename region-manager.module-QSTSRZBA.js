...
var _RegionModalComponent = class _RegionModalComponent extends AsyncHandler {
  ...
  async save() {
    ...
    data.tags = ["region"];
    this.loading = true;
    const resp = await (data.id ? Fc(data.id, __spreadProps(__spreadValues({}, data), {
      name: `REGION ${ve().description} ${data.display_name}`
...
var _RegionManagementService = class _RegionManagementService {
  ...
  async removeRegion(region) {
    ...
    await Lc(region.id).toPromise().catch((e) => {
...
//# sourceMappingURL=region-manager.module-QSTSRZBA.js.map
