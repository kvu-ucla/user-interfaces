...
var _SignageDisplayModalComponent = class _SignageDisplayModalComponent {
  ...
  save() {
    return __async(this, null, function* () {
      ...
      const method = this.display.id ? cc(this.display.id, new_display) : ac(new_display);
...
var _SignageStateService = class _SignageStateService extends AsyncHandler {
  constructor() {
    ...
    this.playlists = combineLatest([
      this._org.active_building,
      this._change
    ]).pipe(filter(([_]) => !!_?.id), debounceTime(300), switchMap(() => Zc({ limit: 500 })), map((_) => (_.data || []).sort((a, b) => a.name.localeCompare(b.name))), shareReplay(1));
    this.displays = combineLatest([
      this._org.active_region,
      this._org.active_building,
      this._change
    ]).pipe(filter(([region, bld]) => !!bld?.id), switchMap(([region, bld]) => oc({
  ...
  editMedia(media = new Ho({}), file, playlist_id = "") {
  ...
  addMedia(_0) {
    return __async(this, arguments, function* (file, media_item = new Ho({})) {
      ...
      const data = __spreadValues({}, new Ho(__spreadProps(__spreadValues({}, media_item), {
      ...
      const result = yield Jc(data).toPromise();
...
var _SignageDisplaysComponent = class _SignageDisplaysComponent extends AsyncHandler {
  constructor() {
    ...
    this.triggers = this.selected.pipe(switchMap((id) => vc(id)), map((_) => _.data), tap((_) => setTimeout(() => this.switching = false, 200)), shareReplay(1));
...
//# sourceMappingURL=signage.module-I5CI7NVG.js.map
