...
var _AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  loadSettings(id) {
    return __async(this, null, function* () {
      ...
      this.settings = { custom: [] };
      const settings = yield nextValueFrom(tc({ parent_id: id }).pipe(map((_) => _.data)));
      const unencrypted = settings.find((_) => _.encryption_level === Pt.None);
      if (!unencrypted)
        return;
      try {
  ...
  save() {
    return __async(this, null, function* () {
      ...
      const settings = yield tc({ parent_id: this.id }).pipe(map((_) => _.data)).toPromise();
      let unencrypted = settings.find((_) => _.encryption_level === Pt.None);
      if (!unencrypted) {
        unencrypted = new St({
          parent_id: this.id,
          encryption_level: Pt.None,
          settings_string: ""
        });
      }
  ...
  saveChanges() {
    return __async(this, null, function* () {
      ...
      const body = __spreadProps(__spreadValues({}, data), {
        tags: ["building"],
        name: `BLD ${ve().description} ${data.display_name}`
      });
      const building = yield (data.id ? Fc(data.id, body) : zc(body)).toPromise().catch((e) => {
...
//# sourceMappingURL=building-manager.module-WYPBXYPU.js.map
