...
var SignageMediaModalComponent = class _SignageMediaModalComponent {
  ...
  saveMedia() {
    return __async(this, null, function* () {
      ...
      if (this.item.id) {
        yield Yc(this.item.id, new_media).toPromise().catch(onError);
      } else {
...
var SignageStateService = class _SignageStateService extends AsyncHandler {
  ...
  constructor(_org, _dialog, _settings) {
    ...
    this.media = combineLatest([
      ...
      this._change
    ]).pipe(filter(([_]) => !!_?.id), debounceTime(300), switchMap(([bld]) => Vc({ limit: 2500 })), map((_) => _.data.sort((a, b) => b.created_at - a.created_at)), shareReplay(1));
  ...
  savePlaylist(playlist) {
    return __async(this, null, function* () {
      const call = playlist.id ? Xc(playlist.id, playlist) : ea(playlist);
  ...
  removePlaylist(playlist) {
    return __async(this, null, function* () {
      ...
      if (result.reason !== "done")
        return;
      yield ta(playlist.id).toPromise();
  ...
  previewFileMedia(media, playlist_id = "") {
    ...
    ref.componentInstance.save.subscribe(() => __async(this, null, function* () {
      ...
      if (playlist_id && new_media.id) {
        const media_list = yield na(playlist_id).toPromise();
        const new_media_list = [...media_list.items, new_media.id];
        yield ia(playlist_id, new_media_list).toPromise();
      }
      ref.close();
    }));
  }
  ...
  updateMedia(item) {
    return __async(this, null, function* () {
      if (!item?.id)
        return;
      yield Yc(item.id, item).toPromise();
    });
  }
  removeMedia(item) {
    return __async(this, null, function* () {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SignageDisplaysComponent, { className: "SignageDisplaysComponent", filePath: "apps/concierge/src/app/signage/signage-displays.component.ts", lineNumber: 239 });
})();
...
var SignageMediaListComponent = class _SignageMediaListComponent {
  ...
  addToPlaylist(media_id, playlist) {
    return __async(this, null, function* () {
      const media_list = yield na(playlist.id).toPromise();
...
var SignagePlaylistMediaListComponent = class _SignagePlaylistMediaListComponent {
  ...
  constructor(_state, _router) {
    ...
    this._playlist_media = this._playlist.pipe(filter((playlist) => !!playlist), debounceTime(300), switchMap((playlist) => na(playlist).pipe(catchError(() => of({ id: "", items: [] })))), shareReplay(1));
    this.media = combineLatest([
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SignagePlaylistMediaListComponent, { className: "SignagePlaylistMediaListComponent", filePath: "apps/concierge/src/app/signage/signage-playlist-media-list.component.ts", lineNumber: 263 });
})();
...
var SignageZonesComponent = class _SignageZonesComponent extends AsyncHandler {
  constructor(_state, _route) {
    ...
    this.triggers = this.selected.pipe(switchMap((id) => Lc(id)), map((_) => _.data), tap((_) => setTimeout(() => this.switching = false, 100)), shareReplay(1));
    this.active_trigger = combineLatest([
      this.triggers,
      this.selected_trigger
...
//# sourceMappingURL=signage.module-BEZI2AHK.js.map
