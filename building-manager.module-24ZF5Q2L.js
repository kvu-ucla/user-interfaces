...
var _InductionSettingsModalComponent = class _InductionSettingsModalComponent {
  ...
  async loadSettings() {
    ...
    const [bld_metadata, org_metadata, org_settings] = await Promise.all([
      await lastValueFrom(hu(this._zone_id, visitor_kiosk_app)),
      await lastValueFrom(hu(this._org.organisation.id, visitor_kiosk_app)),
      await lastValueFrom(hu(this._org.organisation.id, "settings"))
    ]);
  ...
  async save() {
    ...
    const metadata = await lastValueFrom(hu(this._zone_id, visitor_kiosk_app));
    const con_metadata = await lastValueFrom(hu(this._zone_id, concierge_app));
    ...
    const result_visitor = await lastValueFrom(du(this._zone_id, {
    ...
    const result_concierge = await lastValueFrom(du(this._zone_id, {
...
//# sourceMappingURL=building-manager.module-24ZF5Q2L.js.map
