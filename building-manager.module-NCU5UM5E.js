...
var _AutoReleaseSettingsModalComponent = class _AutoReleaseSettingsModalComponent {
  ...
  async loadSettings(id) {
    ...
    this.settings = { custom: [] };
    const settings = await nextValueFrom(tc({ parent_id: id }).pipe(map((_) => _.data)));
  ...
  async save() {
    ...
    const settings = await tc({ parent_id: this.id }).pipe(map((_) => _.data)).toPromise();
    ...
    unencrypted.id ? await lastValueFrom(nc(unencrypted.id, unencrypted)).catch(on_error) : await lastValueFrom(rc(unencrypted)).catch(on_error);
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await lastValueFrom(hu(this.id, metadata_key));
    const details = metadata.details || {};
    details.auto_release = new_settings;
    await nextValueFrom(du(this.id, {
      name: metadata_key,
      details,
      description: ""
    })).catch(on_error);
...
var _BuildingFormComponent = class _BuildingFormComponent extends AsyncHandler {
  ...
  async saveChanges() {
    ...
    const building = await (data.id ? Fc(data.id, body) : zc(body)).toPromise().catch((e) => {
...
var _InductionSettingsModalComponent = class _InductionSettingsModalComponent {
  ...
  async loadSettings() {
    ...
    const [bld_metadata, org_metadata, org_settings] = await Promise.all([
      await hu(this._zone_id, visitor_kiosk_app).toPromise(),
      await hu(this._org.organisation.id, visitor_kiosk_app).toPromise(),
      await hu(this._org.organisation.id, "settings").toPromise()
    ]);
  ...
  async save() {
    ...
    const metadata = await hu(this._zone_id, visitor_kiosk_app).toPromise();
    const con_metadata = await hu(this._zone_id, concierge_app).toPromise();
    ...
    const result = await du(this._zone_id, {
    ...
    const result2 = await du(this._zone_id, {
...
var _ItemListModalComponent = class _ItemListModalComponent {
  ...
  async ngOnInit() {
    const metadata_key = this._settings.get("app.workplace_metadata_key") || "workplace_app";
    const metadata = await hu(this._bld_id, metadata_key).toPromise();
  ...
  async save() {
    ...
    let resp = await du(this._bld_id, {
      name: metadata_key,
    ...
    const concierge_metadata = await hu(this._bld_id, metadata_key).toPromise();
    concierge_metadata.details.support_issue_types = items;
    resp = await du(this._bld_id, {
      name: concierge_key,
...
var _BuildingManagementService = class _BuildingManagementService {
  ...
  async removeBuilding(building) {
    ...
    await Lc(building.id).toPromise().catch((e) => {
...
//# sourceMappingURL=building-manager.module-NCU5UM5E.js.map
