import "./chunk-S3X7D7HG.js";
import "./chunk-ODIEKNNM.js";
import "./chunk-VJ4WC2G2.js";
import "./chunk-POLWT2MK.js";
import {
  ActivatedRoute,
...
var _ReportsStateService = class _ReportsStateService {
  ...
  constructor() {
    ...
    this.counts = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => fu(z, "desks").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const map2 = {};
...
var _LockersReportService = class _LockersReportService {
  constructor() {
    ...
    this.counts$ = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => fu(z, "lockers-spaces").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const mapping = {};
...
var _ParkingReportService = class _ParkingReportService {
  constructor() {
    ...
    this.counts$ = this._options.pipe(debounceTime(500), switchMap((filters) => {
      ...
      return Promise.all(zones.map((z) => fu(z, "parking-spaces").pipe(catchError(() => of({ details: [] })), map((m) => [z, m.details.length])).toPromise()));
    }), map((list) => {
      const mapping = {};
      list.forEach(([id, count]) => mapping[id] = Math.max(count || 0, 1));
      return mapping;
...
//# sourceMappingURL=reports.module-IIHJR3CL.js.map
